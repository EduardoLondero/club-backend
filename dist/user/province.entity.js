var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Entity, Collection, Property, OneToMany, } from '@mikro-orm/core';
import { BaseEntity } from '../shared/baseEntity.entity.js';
import { Locality } from './locality.entity.js';
let Province = class Province extends BaseEntity {
    constructor() {
        super(...arguments);
        this.localities = new Collection(this);
    }
};
__decorate([
    Property({ fieldName: 'nombreProvincia', nullable: false }),
    __metadata("design:type", String)
], Province.prototype, "nameprovince", void 0);
__decorate([
    OneToMany(() => Locality, (locality) => locality.province),
    __metadata("design:type", Object)
], Province.prototype, "localities", void 0);
Province = __decorate([
    Entity({ tableName: 'Provincia', schema: 'dbo' })
], Province);
export { Province };
//# sourceMappingURL=province.entity.js.map